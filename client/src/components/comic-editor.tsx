...
import { Type, MessageSquare, Download, Trash2, X } from 'lucide-react';
...
export default function ComicEditor({ imageUrl, comicId, onSave, onClose }: ComicEditorProps) {
  ...
  const [fontSize, setFontSize] = useState(20);  const [textColor, setTextColor] = useState('#000000');
  ...
  const colorPresets = [
    ...
    if (canvasRef.current && !canvas) {
      ...
      // Save initial state to history - removed since we don't need undo/redo
  ...
  // Update the click handler ref whenever any relevant state changes
  useEffect(() => {
  ...
  const addElement = (x: number, y: number) => {
    ...
    if (selectedTool === 'text') {const textOptions: any = {
        left: x,
        top: y,
        fontSize: fontSize,
        fill: textColor,
      ...
      // Define bubble images as inline SVG data URLs to avoid server issues
      const bubbleImages = {
...
<svg width="200" height="80" viewBox="0 0 200 80" xmlns="http://www.w3.org/2000/svg">
  ...
  <path d="M 60 65 L 50 85 L 45 75 L 40 90 L 35 80 L 80 70 Z" fill="white" stroke="#333" stroke-width="3" stroke-linejoin="round"/>
      ...
      // Calculate scale based on text length for better fit
      ...
      FabricImage.fromURL(bubbleImages[bubbleType], { crossOrigin: 'anonymous' }).then((bubbleImg: any) => {
        // Scale the bubble image
        bubbleImg.scale(baseScale);
        
        // Calculate text width based on bubble size and type
        let textWidth = (bubbleImg.width! * baseScale) - 60; // Base padding
        let maxFontSize = fontSize;
        
        // Adjust text area based on bubble type
        if (bubbleType === 'thought') {
          textWidth = textWidth * 0.8; // Thought bubbles are more circular
          maxFontSize = Math.min(fontSize, 18); // Smaller text for thought bubbles
        } else if (bubbleType === 'shout') {
          textWidth = textWidth * 0.7; // Shout bubbles have irregular shape
          maxFontSize = Math.min(fontSize, 22); // Slightly bigger for shouts
        }
        
        // Create text that fits inside the bubble
        const text = new FabricText(textValue, {
          ...
          originX: 'center',
          originY: 'center',
          width: Math.max(textWidth, 80), // Minimum width
          splitByGrapheme: true,
          // Add stroke if enabled (same as regular text)
          ...(addStroke && {
            stroke: strokeColor,
            strokeWidth: 1,
          }),
        ...
        // Position the bubble image first
        bubbleImg.set({
          originX: 'center',
          originY: 'center',
        });

        // Position text in the center of the bubble with type-specific adjustments
        let textOffsetX = 0;
        let textOffsetY = 0;
        
        if (bubbleType === 'thought') {
          textOffsetY = -15; // Move text up for thought bubbles (avoid the small circles)
        } else if (bubbleType === 'shout') {
          textOffsetY = -5; // Slight adjustment for shout bubbles
        } else {
          textOffsetY = -8; // Speech bubbles need slight upward adjustment (avoid tail)
        }

        // Set text position relative to bubble center
        text.set({
          left: textOffsetX,
          top: textOffsetY,
        });
      ...
      setTextValue('');
      setSelectedTool(null);
      return; // Early return since we're handling async loading
    }
  };
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6 p-4">
      <div className="flex-1">        <Card className="comic-border">
          <CardContent className="p-4">
            ...
            </div><div className="border-4 border-gray-800 rounded-lg overflow-hidden bg-gray-100">
              ...
              <div className="space-y-2">
                <div className="flex items-center space-x-2">
                  <input
                    ...
                    className="rounded"
                  />
                  <Label htmlFor="addStroke" className="text-sm">Add Outline</Label>
                </div>
                ...
                <div className="grid grid-cols-6 gap-1">
                  {colorPresets.map((color) => (
                    <button
                      ...
                      className="w-6 h-6 rounded-full border-2 border-gray-800 hover:scale-110 transition-transform"
            ...
            {selectedTool && (
              <div className="bg-white/80 p-3 rounded-lg comic-border">
                <p className="text-sm font-semibold text-gray-700 mb-2">
                  💡 {selectedTool === 'text' 
                    ? 'Click anywhere on the comic to add text' 
                    : `Click anywhere to add a ${bubbleType} bubble`}
                </p>
                {selectedTool === 'dialogue' && (
                  <div className="mt-2 p-2 bg-gray-100 rounded border">
                    ...
                    <div className="text-center">
                      <span 
                        style={{
                          fontSize: `${Math.min(fontSize, bubbleType === 'thought' ? 18 : bubbleType === 'shout' ? 22 : fontSize)}px`,
                          color: textColor,
                          fontFamily: fontFamily,
                          fontWeight: fontWeight,
                          fontStyle: fontStyle,
                          ...(addStroke && {
                            textShadow: `0.5px 0.5px 0 ${strokeColor}, -0.5px -0.5px 0 ${strokeColor}, 0.5px -0.5px 0 ${strokeColor}, -0.5px 0.5px 0 ${strokeColor}`,
                          }),
                        }}
                      >
